...
export default function App() {
  // --- Ensure session_id is always present in localStorage ---
  useEffect(() => {
    let sessionId = localStorage.getItem('session_id');
    if (!sessionId || typeof sessionId !== 'string' || sessionId.length < 8) {
      // Use crypto.randomUUID if available, else fallback to Math.random
      if (window.crypto && window.crypto.randomUUID) {
        sessionId = window.crypto.randomUUID();
      } else {
        sessionId = 'swc_' + Math.random().toString(36).slice(2) + Date.now().toString(36);
      }
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      if (user) {
        ...
        // Optionally sync font and timezone
  ...
  // Ensure initial theme class is set on mount
  useEffect(() => {
    document.documentElement.classList.remove('dark', 'light', 'custom');
    if (theme === 'dark') {
  ...
  // Remove duplicate useAppNavigate
  // Header avatar (shows if user exists)
  function HeaderAvatar() {
    const { previewBackgroundColor, previewTextColor } = useContext(PreviewColorContext);
    const effectiveBg = previewBackgroundColor ?? backgroundColor;
    const effectiveText = previewTextColor ?? textColor;
    return (
      <Link
        to="/profile"
  ...
  function HeaderLogo() {
    ...
    return (
      <Link to="/" className="logo" style={{ display: "flex", alignItems: "center", textDecoration: "none", color: effectiveText }}>
        <span style={{
          display: "inline-flex",
          alignItems: "center",
          justifyContent: "center",
        ...
        <span className="header-title" style={{
          ...
          letterSpacing: 1,
          color: effectiveText,
          background: stepColor(effectiveBg, theme, 2),
          padding: '0 12px',
          borderRadius: 8,
  ...
  function NotificationButton() {
    ...
    const { notifications, loading, hasUnread, fetchNotifications, handleDismiss, handleNotificationClick, allLoaded } = useNotifications(user);
    const appNavigate = useAppNavigate();

    // Helper to generate notification key
    function getNotificationKey(n, idx) {
      return n.id ? `notif-${n.id}` : `notif-${idx}`;
    }
    ...
    // Get custom backgroundColor and theme from context
                  ...
                  notifications
                    ...
                    .map((n, idx) => (
                      <div
                        ...
                        style={{ background: stepColor(backgroundColor, theme, 1), color: headerButtonTextColor, borderRadius: 7, marginBottom: 10, padding: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
  ...
  function MainContent() {
    ...
    return (
      <Routes>
        ...
        {/* Add other <Route> elements here as needed */}
      </Routes>
    );
  }
  ...
  function AppContent() {
    const { previewBackgroundColor, previewTextColor } = useContext(PreviewColorContext);
    const effectiveBg = previewBackgroundColor ?? backgroundColor ?? "var(--background-color)";
    const effectiveText = previewTextColor ?? textColor ?? "var(--text-color)";
    const { theme } = useContext(ThemeContext);
