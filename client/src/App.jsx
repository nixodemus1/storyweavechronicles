...
export default function App() {
                  ...
                  notifications
                    ...
                    .map((n, idx) => (
                      <div
                        ...
                        style={{ background: stepColor(getComputedStyle(document.documentElement).getPropertyValue('--header-button-color').trim() || headerButtonColor, theme, 1), color: headerButtonTextColor, borderRadius: 7, marginBottom: 10, padding: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
  ...
  // Header logo
  function HeaderLogo() {
  // Use chosen background for logo background, and chosen text color for logo text
  const logoBg = getComputedStyle(document.documentElement).getPropertyValue('--background-color').trim() || '#fff';
  const logoText = getComputedStyle(document.documentElement).getPropertyValue('--text-color').trim() || textColor;
    return (
