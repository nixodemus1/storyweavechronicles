...
export default function App() {
  ...
  // Header logo
  function HeaderLogo() {
  // Use context for instant updates
  const { theme, backgroundColor, textColor } = useContext(ThemeContext);
  return (
    <Link to="/" className="logo" style={{ display: "flex", alignItems: "center", textDecoration: "none", color: textColor }}>
      <span style={{
        display: "inline-flex",
        alignItems: "center",
        justifyContent: "center",
      ...
      <span className="header-title" style={{ fontWeight: 700, fontSize: 24, letterSpacing: 1, color: textColor, background: stepColor(backgroundColor, theme, 2), padding: '0 12px', borderRadius: 8, transition: 'color 0.2s, background 0.2s', userSelect: 'none', maxWidth: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>StoryWeave Chronicles</span>
    </Link>
  );
}
        ...
        <Router>
          ...
          >
            <header
              className="header"
              style={{
                background: stepColor(backgroundColor, theme, 2), // Darker shade for header
